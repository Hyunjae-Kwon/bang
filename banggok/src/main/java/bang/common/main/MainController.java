...
public class MainController {
	...
	public ModelAndView searchAll(CommandMap commandMap, HttpServletRequest request) throws Exception {
		...
		List<Map<String, Object>> trip = tripService.searchTrip(commandMap.getMap(), request);
		 ...
		 /* 추천 여행지 검색*/ List<Map<String, Object>> recom =
		  recommendService.searchRecommend(commandMap.getMap(), request);
		  
		  /*동행 구하기 검색*/ List<Map<String, Object>> together =
		  togetherService.searchTogether(commandMap.getMap(), request);
		  
		 /* 여행 후기 검색 */ List<Map<String, Object>> review =
		  reviewService.searchReview(commandMap.getMap(), request);
		 

		  mv.addObject("keyword", keyword);		
		  mv.addObject("trip", trip); mv.addObject("recom", recom);
		  mv.addObject("together", together); mv.addObject("review", review);
